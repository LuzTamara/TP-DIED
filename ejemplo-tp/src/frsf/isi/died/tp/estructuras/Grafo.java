...
public class Grafo<T> {
    ...
    public ArrayList<T> caminosPosibles(T n1, T n2){
		ArrayList<T> salida = new ArrayList<>();
		ArrayList<T> marcados = new ArrayList<>();
		marcados.add(n1);
		buscarCaminos(n1, n2, marcados, salida);
    	return salida;
    }
    
    private void buscarCaminos(T v1, T v2, ArrayList<T> marcados, ArrayList<T> todos) {
    	ArrayList<T> adyacentes = new ArrayList<>();
    	ArrayList<T> copiaMarcados = new ArrayList<>();
    	adyacentes = (ArrayList<T>) this.getAdyacentes(v1);
    	for(T ady : adyacentes) {
    		if(v2.equals(ady)) {
    			copiaMarcados = (ArrayList<T>) marcados.clone();
    			copiaMarcados.add(v2);
    			todos.addAll(copiaMarcados);
    		}
    		else {
    			if(!marcados.contains(ady)) {
    				copiaMarcados = (ArrayList<T>) marcados.clone();
    				copiaMarcados.add(ady);
    				buscarCaminos(ady, v2, copiaMarcados, todos);
    			}
    		}
    	}
    }
    
    public List<T> encontrarNodosEntrada(T v1) {
    	Vertice<T> vertice = this.getNodo(v1);
		List<T> nodosEntrantes = new ArrayList<T>();
		for(Arista<T> arista : this.aristas){
			if(arista.getFin().equals(vertice)) {
				nodosEntrantes.add(arista.getInicio().getValor());
			}
		}
    	return nodosEntrantes;
    }
